...
def define_network(x, keep_prob):
    ...
    with tf.name_scope("fc_2_1"):
        W_fc_2_1 = weight_variable([1024, 10])
        b_fc_2_1 = bias_variable([10])
        y_conv_1 = tf.matmul(h_fc_1_drop, W_fc_2_1) + b_fc_2_1

    with tf.name_scope("fc_2_2"):
        W_fc_2_2 = weight_variable([1024, 10])
        b_fc_2_2 = bias_variable([10])
        y_conv_2 = tf.matmul(h_fc_1_drop, W_fc_2_2) + b_fc_2_2

    return y_conv_1, y_conv_2
...
def train():
    ...
    y_1, y_2 = define_network(x, keep_prob)
    ...
    with tf.name_scope('cross_entropy'):
        ...
        with tf.name_scope('total'):
            cross_entropy_1 = tf.losses.sparse_softmax_cross_entropy(
                labels=y_[:, 0], logits=y_1)
            cross_entropy_2 = tf.losses.sparse_softmax_cross_entropy(
                labels=y_[:, 1], logits=y_2)
            cross_entropy_1 = tf.reduce_mean(cross_entropy_1)
            cross_entropy_2 = tf.reduce_mean(cross_entropy_2)
            cross_entropy = (cross_entropy_1 + cross_entropy_2) / 2.0
    ...
    with tf.name_scope('accuracy'):
        with tf.name_scope('correct_prediction'):
            predict_1 = tf.argmax(y_1, 1)
            predict_2 = tf.argmax(y_2, 1)
            correct_prediction = tf.abs((predict_1 + predict_2) - (y_[:, 0] + y_[:, 1]))
        with tf.name_scope('accuracy'):
